#include "stringGenerateItemWidget.h"
...
StringGenerateItemWidget::StringGenerateItemWidget( QWidget *parent, const Qt::WindowFlags &f ) : QWidget( parent, f ) {
...
void StringGenerateItemWidget::setInfo( const size_t &index, const QString &var_value ) {
	auto title = infoFormmattion.arg( QString::number( index ) );
	showIndexInfo->setText( title );
	editorVarInfo->setText( var_value );
}
void StringGenerateItemWidget::setIndex( const size_t &index ) {
	auto title = infoFormmattion.arg( QString::number( index ) );
	showIndexInfo->setText( title );
}
bool StringGenerateItemWidget::isDragWidgetPos( const QPoint &pos ) {
	if( geometry( ).contains( pos ) )
		return true;
	return false;
}
QString StringGenerateItemWidget::getVarValue( ) const { return editorVarInfo->text( ); }
