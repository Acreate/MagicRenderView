#include "binGenerateItemWidget.h"
...
BinGenerateItemWidget::BinGenerateItemWidget( QWidget *parent, const Qt::WindowFlags &f ) : QWidget( parent, f ) {
	...
	infoFormmattion = QString( tr( "bin [ %1 ]" ) );
...
void BinGenerateItemWidget::setInfo( const size_t &index, const QString &var_value ) {
	auto title = infoFormmattion.arg( QString::number( index ) );
	showIndexInfo->setText( title );
	editorVarInfo->setText( var_value );
}
void BinGenerateItemWidget::setIndex( const size_t &index ) {
	auto title = infoFormmattion.arg( QString::number( index ) );
	showIndexInfo->setText( title );
}
bool BinGenerateItemWidget::isDragWidgetPos( const QPoint &pos ) {
	if( geometry( ).contains( pos ) )
		return true;
	return false;
}
QString BinGenerateItemWidget::getVarValue( ) const { return editorVarInfo->text( ); }
