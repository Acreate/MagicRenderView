#include "colorGenerateItemWidget.h"
...
ColorGenerateItemWidget::ColorGenerateItemWidget( QWidget *parent, const Qt::WindowFlags &f ) : QWidget( parent, f ) {
	...
	infoFormmattion = QString( tr( "color [ %1 ]" ) );
...
void ColorGenerateItemWidget::setInfo( const size_t &index, const QString &var_value ) {
	auto title = infoFormmattion.arg( QString::number( index ) );
	showIndexInfo->setText( title );
	editorVarInfo->setText( var_value );
}
void ColorGenerateItemWidget::setIndex( const size_t &index ) {
	auto title = infoFormmattion.arg( QString::number( index ) );
	showIndexInfo->setText( title );
}
bool ColorGenerateItemWidget::isDragWidgetPos( const QPoint &pos ) {
	if( geometry( ).contains( pos ) )
		return true;
	return false;
}
QString ColorGenerateItemWidget::getVarValue( ) const { return editorVarInfo->text( ); }
