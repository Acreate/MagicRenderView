#include "imageGenerateItemWidget.h"
...
ImageGenerateItemWidget::ImageGenerateItemWidget( QWidget *parent, const Qt::WindowFlags &f ) : QWidget( parent, f ) {
...
void ImageGenerateItemWidget::setInfo( const size_t &index, const QString &var_value ) {
	auto title = infoFormmattion.arg( QString::number( index ) );
	showIndexInfo->setText( title );
	editorVarInfo->setText( var_value );
}
void ImageGenerateItemWidget::setIndex( const size_t &index ) {
	auto title = infoFormmattion.arg( QString::number( index ) );
	showIndexInfo->setText( title );
}
bool ImageGenerateItemWidget::isDragWidgetPos( const QPoint &pos ) {
	if( geometry( ).contains( pos ) )
		return true;
	return false;
}
QString ImageGenerateItemWidget::getVarValue( ) const { return editorVarInfo->text( ); }
