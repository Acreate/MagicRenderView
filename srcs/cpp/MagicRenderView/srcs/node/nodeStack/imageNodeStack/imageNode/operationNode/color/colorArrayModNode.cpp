...
Def_Entity_NodeTypeName_Function( ColorArrayModNode, Node::tr( "颜色/序列/求余" ) );
...
bool ColorArrayModNode::updateLayout( ) {
	if( ProcessNode::updateLayout( ) == false )
		return false;
	return true;
}
bool ColorArrayModNode::readyNodeRunData( ) {
	return true;
}
bool ColorArrayModNode::fillNodeCall( const QDateTime &ndoe_run_start_data_time, size_t current_frame ) {
	...
	*outputVarPtr = *converInt;
	...
	for( index = 0; index < count; index += 1 )
		ImageTools::colorOperation::mod( outputArrayPtr[ index ], red, green, blue, alpha );
	return true;
}
