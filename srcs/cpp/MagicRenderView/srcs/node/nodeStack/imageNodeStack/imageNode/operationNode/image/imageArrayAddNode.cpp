...
Def_Entity_NodeTypeName_Function( ImageArrayAddNode, Node::tr( "图像/序列/加法" ) );

ImageArrayAddNode::ImageArrayAddNode( const QString &node_name ) : ProcessNode( node_name ) {
	outputVarPtr = nullptr;
}
bool ImageArrayAddNode::initEx( MainWidget *parent ) {
	initExCallFunction = [this] ( MainWidget *draw_node_widget ) {
		if( nodeComponentControlPtr->appendInputPortType( this, tr( "图像序列" ), firstInputPort ) == false )
			return false;
		if( nodeComponentControlPtr->appendInputPortType( this, tr( "图像" ), secondInputPort ) == false )
			return false;
...
bool ImageArrayAddNode::updateLayout( ) {
...
bool ImageArrayAddNode::fillNodeCall( const QDateTime &ndoe_run_start_data_time, size_t current_frame ) {
	...
	*outputVarPtr = *converInt;
	...
	for( index = 0; index < count; index += 1 ) {
		...
		ImageTools::imageOperation::add( image, *secondConverPtr, minWidth > width ? width : minWidth, minHeight > height ? height : minHeight );
	}
