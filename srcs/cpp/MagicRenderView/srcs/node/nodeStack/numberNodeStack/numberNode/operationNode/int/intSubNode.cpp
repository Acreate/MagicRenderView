...
IntSubNode::IntSubNode( const QString &node_name ) : ProcessNode( node_name ) {
	outputVarPtr = nullptr;
}
bool IntSubNode::initEx( MainWidget *parent ) {
	initExCallFunction = [this] ( MainWidget *draw_node_widget ) {

		if( nodeComponentControlPtr->appendInputPortType( this, tr( "整数" ), firstInputPort ) == false )
			return false;
...
bool IntSubNode::updateLayout( ) {
	if( ProcessNode::updateLayout( ) == false )
		return false;
	return true;
}
bool IntSubNode::readyNodeRunData( ) {
	return true;
}
bool IntSubNode::fillNodeCall( const QDateTime &ndoe_run_start_data_time, size_t current_frame ) {
