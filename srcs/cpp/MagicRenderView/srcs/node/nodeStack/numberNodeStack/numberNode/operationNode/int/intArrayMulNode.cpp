...
Def_Entity_NodeTypeName_Function( IntArrayMulNode, Node::tr( "运算/序列/整数/乘法" ) );

IntArrayMulNode::IntArrayMulNode( const QString &node_name ) : ProcessNode( node_name ) {
	outputVarPtr = nullptr;
}
bool IntArrayMulNode::initEx( MainWidget *parent ) {
	initExCallFunction = [this] ( MainWidget *draw_node_widget ) {
		if( nodeComponentControlPtr->appendInputPortType( this, tr( "整数序列" ), firstInputPort ) == false )
			return false;

		if( nodeComponentControlPtr->appendInputPortType( this, tr( "整数" ), secondInputPort ) == false )
			return false;
...
bool IntArrayMulNode::updateLayout( ) {
	if( ProcessNode::updateLayout( ) == false )
		return false;
	return true;
}
bool IntArrayMulNode::readyNodeRunData( ) {
	return true;
}
bool IntArrayMulNode::fillNodeCall( const QDateTime &ndoe_run_start_data_time, size_t current_frame ) {
