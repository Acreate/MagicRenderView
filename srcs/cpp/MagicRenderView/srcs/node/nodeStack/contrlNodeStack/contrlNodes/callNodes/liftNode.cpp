...
#include "../../../../port/inputPort/call/callInputPort.h"
#include "../../../../port/inputPort/function/functionInputPort.h"
...
bool LiftNode::initEx( MainWidget *parent ) {
	initExCallFunction = [this] ( MainWidget *draw_node_widget ) {
		...
		if( nodeComponentControlPtr->appendInputPortType( this, tr( "提起" ), callInputPortPtr ) == false )
			return false;
		
		if( nodeComponentControlPtr->appendInputPortType( this, tr( "提起钩子" ), functionInputPortPtr ) == false )
			return false;
		return true;
	};
...
bool LiftNode::fillInputPortCall( const QDateTime &ndoe_run_start_data_time, size_t current_frame, std::vector< Node * > &result_need_run_ref_node_vector ) {
	return nodeComponentControlPtr->getRefPortNodeVector( callInputPortPtr, result_need_run_ref_node_vector );
}
...
bool LiftNode::fillOutputPortCall( const QDateTime &ndoe_run_start_data_time, size_t current_frame, std::vector< Node * > &result_next_run_advise_node_vector ) {
	return nodeComponentControlPtr->getFilterRefPortNodeVector( functionInputPortPtr, result_next_run_advise_node_vector, NodeEnum::NodeType::Function );
}
