...
bool StringArrayJoinToStringNode::initEx( MainWidget *parent ) {
	initExCallFunction = [this] ( MainWidget *draw_node_widget ) {
		if( nodeComponentControlPtr->appendInputPortType( this, tr( "原始字符串" ), stringVectorInputPortPtr ) == false )
			return false;
		if( nodeComponentControlPtr->appendInputPortType( this, tr( "拼接子字符串" ), jionSubStringInputPortPtr ) == false )
			return false;
		if( nodeComponentControlPtr->appendOutputPortType( this, tr( "合并字符串" ), outputPortPtr, outputVarPtr ) == false )
			return false;
		outputVarPtr->clear( );
		return true;
	};
...
bool StringArrayJoinToStringNode::fillNodeCall( const QDateTime &ndoe_run_start_data_time, size_t current_frame ) {
	...
	if( nodeComponentControlPtr->cast_ptr_ref_first_port_var_ptr( stringVectorInputPortPtr, stringList ) == false )
		return true;
	...
	QString *joinString;
	if( nodeComponentControlPtr->cast_ptr_ref_first_port_var_ptr( jionSubStringInputPortPtr, joinString ) == false )
		return true;
	count -= 1;
