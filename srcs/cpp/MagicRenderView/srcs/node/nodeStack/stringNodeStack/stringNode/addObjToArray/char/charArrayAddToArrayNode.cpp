...
Def_Entity_NodeTypeName_Function( CharArrayAddToArrayNode, Node::tr( "序列追加/序列/字符序列追加到字符串序列" ) );
...
bool CharArrayAddToArrayNode::initEx( MainWidget *parent ) {
	initExCallFunction = [this] ( MainWidget *draw_node_widget ) {
		if( nodeComponentControlPtr->appendInputPortType( this, tr( "字符序列" ), firstInputPort ) == false )
			return false;
		if( nodeComponentControlPtr->appendInputPortType( this, tr( "字符序列" ), secondInputPort ) == false )
			return false;
