...
ColorArraySubNode::ColorArraySubNode( const QString &node_name ) : ArrayNode( node_name ) {
	outputVarPtr = nullptr;
}
bool ColorArraySubNode::initEx( MainWidget *parent ) {
...
bool ColorArraySubNode::updateLayout( ) {
	if( ArrayNode::updateLayout( ) == false )
		return false;
	return true;
}
bool ColorArraySubNode::readyNodeRunData( ) {
	return true;
}
bool ColorArraySubNode::fillNodeCall( const QDateTime &ndoe_run_start_data_time ) {
	...
	*outputVarPtr = *converInt;
	...
	for( index = 0; index < count; index += 1 )
		ImageTools::colorOperation::sub( outputArrayPtr[ index ], red, green, blue, alpha );
	return true;
}
