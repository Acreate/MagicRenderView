...
CharGenerateNode::CharGenerateNode( const QString &node_name ) : GenerateNode( node_name ) {
	arrayCount = nullptr;
	arrayIndex = nullptr;
...
bool CharGenerateNode::initEx( MainWidget *parent ) {
...
bool CharGenerateNode::initArrayUintyTypeName( QString &change_array_unty_type_name ) {
	return varDirector->getTypeName( typeid( TGenerateType ), change_array_unty_type_name );
}
bool CharGenerateNode::updateLayout( ) {
	if( GenerateNode::updateLayout( ) == false )
		return false;
	return true;
}
std::vector< CharGenerateNode::TGenerateType > * CharGenerateNode::getGenerateVarPtr( ) const {
	return overVarPtr;
}
bool CharGenerateNode::formUint8ArrayData( size_t &result_use_count, const uint8_t *source_array_ptr, const size_t &source_array_count ) {
...
bool CharGenerateNode::toUint8VectorData( std::vector< uint8_t > &result_vector_data ) {
	bool uint8VectorData = GenerateNode::toUint8VectorData( result_vector_data );
	return uint8VectorData;
}
bool CharGenerateNode::initVarPtr( ) {
...
bool CharGenerateNode::readyNodeRunData( ) {
	return true;
}
bool CharGenerateNode::fillNodeCall( const QDateTime &ndoe_run_start_data_time ) {
