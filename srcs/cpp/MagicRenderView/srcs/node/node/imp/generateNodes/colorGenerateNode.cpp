...
ColorGenerateNode::ColorGenerateNode( const QString &node_name ) : GenerateNode( node_name ) {
	arrayCount = nullptr;
	arrayIndex = nullptr;
...
bool ColorGenerateNode::initEx( MainWidget *parent ) {
	initExCallFunction = [this] ( MainWidget *draw_node_widget ) {
		generateInputPort = appendInputPortType< GenerateInputPort >( tr( "生成" ) );
		if( generateInputPort == nullptr )
			return false;
		outputVarPort = appendOutputPortType< ColorOutputPort >( tr( "导出值" ) );
...
bool ColorGenerateNode::initArrayUintyTypeName( QString &change_array_unty_type_name ) {
	return varDirector->getTypeName( typeid( TGenerateType ), change_array_unty_type_name );
}
bool ColorGenerateNode::updateLayout( ) {
	if( GenerateNode::updateLayout( ) == false )
		return false;
	return true;
}
std::vector< ColorGenerateNode::TGenerateType > * ColorGenerateNode::getGenerateVarPtr( ) const {
	return overVarPtr;
}
bool ColorGenerateNode::formUint8ArrayData( size_t &result_use_count, const uint8_t *source_array_ptr, const size_t &source_array_count ) {
...
bool ColorGenerateNode::toUint8VectorData( std::vector< uint8_t > &result_vector_data ) {
	bool uint8VectorData = GenerateNode::toUint8VectorData( result_vector_data );
	return uint8VectorData;
}
bool ColorGenerateNode::initVarPtr( ) {
...
bool ColorGenerateNode::readyNodeRunData( ) {
	return true;
}
bool ColorGenerateNode::fillNodeCall( const QDateTime &ndoe_run_start_data_time ) {
