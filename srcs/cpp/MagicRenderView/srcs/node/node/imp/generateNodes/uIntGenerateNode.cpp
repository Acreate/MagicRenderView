...
UIntGenerateNode::UIntGenerateNode( const QString &node_name ) : GenerateNode( node_name ) {
	arrayCount = nullptr;
	arrayIndex = nullptr;
...
bool UIntGenerateNode::initEx( MainWidget *parent ) {
	initExCallFunction = [this] ( MainWidget *draw_node_widget ) {
		generateInputPort = appendInputPortType< GenerateInputPort >( tr( "生成" ) );
		if( generateInputPort == nullptr )
			return false;
		outputVarPort = appendOutputPortType< UIntOutputPort >( tr( "导出值" ) );
...
bool UIntGenerateNode::initArrayUintyTypeName( QString &change_array_unty_type_name ) {
	return varDirector->getTypeName( typeid( TGenerateType ), change_array_unty_type_name );
}
bool UIntGenerateNode::updateLayout( ) {
	if( GenerateNode::updateLayout( ) == false )
		return false;
	return true;
}
std::vector< UIntGenerateNode::TGenerateType > * UIntGenerateNode::getGenerateVarPtr( ) const {
	return overVarPtr;
}
bool UIntGenerateNode::formUint8ArrayData( size_t &result_use_count, const uint8_t *source_array_ptr, const size_t &source_array_count ) {
...
bool UIntGenerateNode::toUint8VectorData( std::vector< uint8_t > &result_vector_data ) {
	bool uint8VectorData = GenerateNode::toUint8VectorData( result_vector_data );
	return uint8VectorData;
}
bool UIntGenerateNode::initVarPtr( ) {
...
bool UIntGenerateNode::readyNodeRunData( ) {
	return true;
}
bool UIntGenerateNode::fillNodeCall( const QDateTime &ndoe_run_start_data_time ) {
