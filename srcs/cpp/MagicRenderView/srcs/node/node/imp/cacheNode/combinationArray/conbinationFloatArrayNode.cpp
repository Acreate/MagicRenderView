#include "conbinationFloatArrayNode.h"
...
#include "../../../../port/inputPort/cache/floatCacheInputPort.h"
#include "../../../../port/outputPort/array/floatVectorOutputPort.h"

ConbinationFloatArrayNode::ConbinationFloatArrayNode( const QString &node_name ) : CacheNode( node_name ) {
	outVarPtr = nullptr;
}
bool ConbinationFloatArrayNode::initEx( MainWidget *parent ) {
	initExCallFunction = [this] ( MainWidget *draw_node_widget ) {
		
		if( appendInputPortType( tr( "浮点" ), cacheInputPort ) == false )
			return false;
		if( appendInputPortType( tr( "条件" ), anyInputPort ) == false )
			return false;
		if( appendOutputPortType( tr( "浮点序列" ), vectorOutPortPtr ) == false )
			return false;
...
bool ConbinationFloatArrayNode::updateLayout( ) {
	if( CacheNode::updateLayout( ) == false )
		return false;
	return true;
}
bool ConbinationFloatArrayNode::readyNodeRunData( ) {
	return true;
}

bool ConbinationFloatArrayNode::fillNodeCall( const QDateTime &ndoe_run_start_data_time ) {
