#include "conbinationColorArrayNode.h"
...
#include "../../../../port/inputPort/cache/colorCacheInputPort.h"
#include "../../../../port/outputPort/array/colorVectorOutputPort.h"

ConbinationColorArrayNode::ConbinationColorArrayNode( const QString &node_name ) : CacheNode( node_name ) {
	outVarPtr = nullptr;
}
bool ConbinationColorArrayNode::initEx( MainWidget *parent ) {
	initExCallFunction = [this] ( MainWidget *draw_node_widget ) {

		if( appendInputPortType( tr( "颜色" ), cacheInputPort ) == false )
			return false;
		if( appendInputPortType( tr( "条件" ), anyInputPort ) == false )
			return false;
		if( appendOutputPortType( tr( "颜色序列" ), vectorOutPortPtr ) == false )
			return false;
...
bool ConbinationColorArrayNode::updateLayout( ) {
	if( CacheNode::updateLayout( ) == false )
		return false;
	return true;
}
bool ConbinationColorArrayNode::readyNodeRunData( ) {
	return true;
}

bool ConbinationColorArrayNode::fillNodeCall( const QDateTime &ndoe_run_start_data_time ) {
