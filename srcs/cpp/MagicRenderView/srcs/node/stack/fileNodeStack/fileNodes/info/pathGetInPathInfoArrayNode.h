#ifndef PATHGETINPATHINFOARRAYNODE_H_H_HEAD__FILE__
#define PATHGETINPATHINFOARRAYNODE_H_H_HEAD__FILE__

#include <node/node/processNode.h>

class StringVectorOutputPort;
class StringInputPort;
class PathGetInPathInfoArrayNode : public ProcessNode {
	Q_OBJECT;
protected:
...
public:
	PathGetInPathInfoArrayNode( const QString &node_name );
	bool initEx( MainWidget *parent ) override;
	bool updateLayout( ) override;
public:
	bool fillNodeCall( const QDateTime &ndoe_run_start_data_time, size_t current_frame ) override;
public:
	Def_Extend_NodeTypeName( Node::tr( "路径/递归获取文件与目录" ) );
};
#endif // PATHGETINPATHINFOARRAYNODE_H_H_HEAD__FILE__
