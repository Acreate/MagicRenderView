...
Def_Entity_NodeTypeName_Function( BinArraySubNode, Node::tr( "运算/序列/二进制/减法" ) );

BinArraySubNode::BinArraySubNode( const QString &node_name ) : ProcessNode( node_name ) {
	outputVarPtr = nullptr;
}
bool BinArraySubNode::initEx( MainWidget *parent ) {
...
bool BinArraySubNode::updateLayout( ) {
	if( ProcessNode::updateLayout( ) == false )
		return false;
	return true;
}
bool BinArraySubNode::readyNodeRunData( ) {
	return true;
}
bool BinArraySubNode::fillNodeCall( const QDateTime &ndoe_run_start_data_time, size_t current_frame ) {
