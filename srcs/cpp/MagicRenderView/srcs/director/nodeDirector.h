...
Q_SIGNALS:
	...
	/// @param target_prot 释放的目标端口对象指针
	/// @param srack_info 堆栈信息
	void finish_release_node_port_link_signal( InputPort *signal_port, OutputPort *target_prot, const SrackInfo &srack_info );
	/// @brief 节点端口发生链接时，产生该信号
	/// @param signal_port 链接的源端口对象指针
	/// @param target_prot 链接的目标端口对象指针
	/// @param srack_info 堆栈信息
	void finish_create_node_port_link_signal( InputPort *signal_port, OutputPort *target_prot, const SrackInfo &srack_info );
	/// @brief 节点依赖发生释放时候，产生该信号
	/// @param signal_node 依赖节点
	/// @param ref_node 被删除的依赖
	/// @param srack_info 堆栈信息
	void finish_release_ref_node_signal( Node *signal_node, Node *ref_node, const SrackInfo &srack_info );
	/// @brief 节点依赖发生增持时候，产生该信号
	/// @param signal_node 依赖节点
	/// @param ref_node 被增持的依赖
	/// @param srack_info 堆栈信息
	void finish_create_ref_node_signal( Node *signal_node, Node *ref_node, const SrackInfo &srack_info );
	/// @brief 节点错误信号
	...
	/// @param srack_info 堆栈信息
	void finish_create_node_signal( const QString &create_name, const SrackInfo &srack_info );
	/// @brief 节点创建失败信号
	/// @param create_name 节点名称
	/// @param error_type_info 错误类型
	/// @param error_msg 失败信息
	/// @param srack_info 堆栈信息
	void error_create_node_signal( const QString &create_name, NodeEnum::CreateType error_type_info, const QString &error_msg, const SrackInfo &srack_info );
};
