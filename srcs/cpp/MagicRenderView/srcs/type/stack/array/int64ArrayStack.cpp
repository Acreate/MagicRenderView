...
uint64_t Int64ArrayStack::toObj( const uint8_t *obj_start_ptr, const size_t &obj_memory_size, void *&result_obj_ptr ) {
	...
	offset = offset + count;
	mod = mod - count;
	std::vector< int64_t > buffVar( arrayCount );

	auto arrayPtr = buffVar.data( );
	for( size_t index = 0; index < arrayCount; ++index ) {
		int64_t *ptr = arrayPtr + index;
		count = fillTypeVectorAtVar< int64_t >( offset, mod, ptr );
		if( count == 0 )
			return 0;
		offset = offset + count;
		mod = mod - count;
	...
	auto createPtr = ( std::vector< int64_t > * ) createTypePtr( );
	*createPtr = buffVar;
...
uint64_t Int64ArrayStack::toVectorData( void *obj_start_ptr, std::vector< uint8_t > &result_data ) {
	std::vector< uint8_t > buff;
	uint64_t count;
	count = getTypeNameAtData( result_data );
	if( count == 0 )
		return 0;
	std::vector< int64_t > *vector = ( std::vector< int64_t > * ) obj_start_ptr;
	...
	for( size_t index = 0; index < arraySize; ++index ) {
		int64_t *ptr = arrayPtr + index;
		count = fillTypeVarAtVector< int64_t >( ptr, buff );
		if( count == 0 )
			return 0;
		result_data.append_range( buff );
	}
