...
UInt32ArrayStack::UInt32ArrayStack( ) {
	...
	deleteObjTypeFunction = [] ( void *delete_obj_ptr ) {
		delete ( std::vector< uint32_t > * ) delete_obj_ptr;
		return true;
	};
}

uint64_t UInt32ArrayStack::toObj( const uint8_t *obj_start_ptr, const size_t &obj_memory_size, void *&result_obj_ptr ) {
	...
	offset = offset + count;
	mod = mod - count;
	std::vector< uint32_t > buffVar( arrayCount );

	auto arrayPtr = buffVar.data( );
	for( size_t index = 0; index < arrayCount; ++index ) {
		uint32_t *ptr = arrayPtr + index;
		count = fillTypeVectorAtVar< uint32_t >( offset, mod, ptr );
		if( count == 0 )
			return 0;
		offset = offset + count;
		mod = mod - count;
	...
	auto createPtr = ( std::vector< uint32_t > * ) createTypePtr( );
	*createPtr = buffVar;
...
uint64_t UInt32ArrayStack::toVectorData( void *obj_start_ptr, std::vector< uint8_t > &result_data ) {
	std::vector< uint8_t > buff;
	uint64_t count;
	count = getTypeNameAtData( result_data );
	if( count == 0 )
		return 0;
	std::vector< uint32_t > *vector = ( std::vector< uint32_t > * ) obj_start_ptr;
	...
	for( size_t index = 0; index < arraySize; ++index ) {
		uint32_t *ptr = arrayPtr + index;
		count = fillTypeVarAtVector< uint32_t >( ptr, buff );
		if( count == 0 )
			return 0;
		result_data.append_range( buff );
	}
