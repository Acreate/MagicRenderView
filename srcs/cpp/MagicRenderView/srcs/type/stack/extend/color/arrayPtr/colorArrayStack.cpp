...
bool ColorArrayStack::init( VarDirector *var_director ) {
	if( InfoStack::init( var_director ) == false )
		return false;
	Stack_Type_Name( , std::vector< QColor >, "vector<QColor>", "QColor[]", "QColorArray", "vector<Color>", "Color[]", "ColorArray" );
	return true;
}
ColorArrayStack::ColorArrayStack( ) {
}

bool ColorArrayStack::toObj( uint64_t &result_count, const uint8_t *obj_start_ptr, const size_t &obj_memory_size, void *&result_obj_ptr ) {
...
TypeEnum::Type ColorArrayStack::getType( ) {
	return TypeEnum::Type::Array;
}
bool ColorArrayStack::toVectorData( void *obj_start_ptr, std::vector< uint8_t > &result_data ) {
