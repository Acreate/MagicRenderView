...
using t_current_unity_type = bool;
bool BoolArrayStack::toObj( uint64_t &result_count, const uint8_t *obj_start_ptr, const size_t &obj_memory_size, void *&result_obj_ptr ) {
	...
	auto arrayPtr = new bool[ arrayCount ];
	...
	if( hasVarPtr( result_obj_ptr ) == false ) {
		...
		if( varDirector->create( sourcePtr ) == false || sourcePtr == nullptr )
			return false;
		*sourcePtr = std::vector< bool >( arrayPtr, arrayPtr + arrayCount );
		result_obj_ptr = sourcePtr;
		delete []arrayPtr;
		return true;
	}
	auto createPtr = ( std::vector< t_current_unity_type > * ) result_obj_ptr;
	*createPtr = std::vector< bool >( arrayPtr, arrayPtr + arrayCount );
	delete []arrayPtr;
	return true;
}
TypeEnum::Type BoolArrayStack::getType( ) {
	return TypeEnum::Type::Array;
}
bool BoolArrayStack::toVectorData( void *obj_start_ptr, std::vector< uint8_t > &result_data ) {
	...
	auto arrayPtr = new bool[ arraySize ];
	size_t index;
	for( index = 0; index < arraySize; ++index )
		arrayPtr[ index ] = vector->at( index );
	for( index = 0; index < arraySize; ++index )
