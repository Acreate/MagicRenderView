...
#define generateMenuVector_emplace_back( type_, type_name , ...) \
	generateMenuVector.emplace_back([] ( QWidget *parent ) ->NormalGenerateNodeMenu * { \
...
NormalGenerateNodeMenu * GenerateNodeMenuStack::createGenerateNodeMenu( QWidget *parent, const QString &menu_name ) {
	size_t index;
	for( index = 0; index < generateCount; ++index )
		if( generateArrayPtr[ index ].second.first == menu_name )
			return generateArrayPtr[ index ].first( parent );
	QString *stringArray;
	size_t stringCount;
	size_t aliasIndex;
	for( index = 0; index < generateCount; ++index )
