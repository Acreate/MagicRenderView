...
bool NodePort::renderLayout( bool ico_is_end ) {
	...
	if( ico->height( ) != portItemHeith ) {
		*ico = ico->scaledToHeight( portItemHeith );
		if( ico->isNull( ) ) {
			tools::debug::printError( QString( QObject::tr( "适配图标失败[%1]" ) ).arg( getMetaObjectPathName( ) ) );
			return false;
		}
	}
	...
	*nodePortRender = nodePortRender->scaled( portItemWidth, portItemHeith );
	...
	if( nodePortRender->isNull( ) ) {
		tools::debug::printError( QString( QObject::tr( "渲染标题失败[%1]" ) ).arg( getMetaObjectPathName( ) ) );
		return false;
	}
	return true;
}
