#include "qStringStack.h"

QStringStack::QStringStack( ) : I_Stack( typeid( t_current_type ) ) {

}

bool QStringStack::toBinVector( const type_info &target_type_info, const void *target_ptr, std_vector< uint8_t > &result_vector, size_t &result_count ) const {
	...
	typeName = *( QString * ) target_ptr;
	count += fillBinVector( typeName, buff );
...
bool QStringStack::toOBjVector( const type_info &target_type_info, void *target_ptr, size_t &result_count, const uint8_t *source_data_ptr, const size_t &source_data_count ) const {
