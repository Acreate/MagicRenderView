...
AppNodeItemStack::AppNodeItemStack( ) : I_Stack( typeid( t_current_type ) ), releaseVector( new std_vector_pairt< void *, std_function< bool( void * ) > > ) {
...
bool AppNodeItemStack::toBinVector( const type_info &target_type_info, const void *target_ptr, std_vector< uint8_t > &result_vector, size_t &result_count ) const {
	...
	count = targetPtr->nodeVarVector.size( );
	// 记录链接窗口接口个数
	result_count = fillBinVector( &count, sizeof( size_t ), inputPortVectorBuff );
	if( result_count == 0 )
		return false; // 失败，返回
	if( count != 0 ) {
		auto data = targetPtr->nodeVarVector.data( );
		size_t index = 0;
		std_vector< uint8_t > linkPortVectorBuff;
		for( ; index < count; ++index ) {
...
bool AppNodeItemStack::toOBjVector( const type_info &target_type_info, void *target_ptr, size_t &result_count, const uint8_t *source_data_ptr, const size_t &source_data_count ) const {
	...
	mod -= result_count;
	if( count > 0 ) {
		nodeItem->nodeVarVector.resize( count );
		auto data = nodeItem->nodeVarVector.data( );
		size_t index = 0;
		for( ; index < count; ++index ) {
			result_count = fillObjVector( &needCount, sizeof( size_t ), offset, mod );
			if( result_count == 0 )
