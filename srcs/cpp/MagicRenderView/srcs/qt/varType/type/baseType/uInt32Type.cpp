...
bool UInt32Type::createTarget( const type_info &target_type_info, const std_function< void( void *create_obj_ptr ) > &create_call_function ) {
...
bool UInt32Type::deleteTarget( const type_info &target_type_info, void *target_ptr ) {
	if( target_type_info != generateTypeInfo )
		return false;
	return deleteTarget( target_ptr );
}
bool UInt32Type::deleteTarget( void *target_ptr ) {
...
bool UInt32Type::toBinVector( const type_info &target_type_info, const void *target_ptr, std_vector< uint8_t > &result_vector, size_t &result_count ) {
	if( target_type_info != generateTypeInfo )
		return false;
...
bool UInt32Type::toOBjVector( const type_info &target_type_info, void *target_ptr, size_t &result_count, const uint8_t *source_data_ptr, const size_t &source_data_count ) {
	if( target_type_info != generateTypeInfo )
		return false;
	size_t needCount = 0;
