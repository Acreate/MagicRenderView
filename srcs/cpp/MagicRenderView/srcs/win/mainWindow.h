...
protected Q_SLOTS:
	...
	void release_node_slot( NodeDirector *signal_obj_ptr, Node *release_node, const SrackInfo &srack_info );
	/// @brief 节点端口发生释放时，产生该信号
	/// @param signal_obj_ptr 信号对象指针
	/// @param signal_port 释放的源端口对象指针
	/// @param target_prot 释放的目标端口对象指针
	/// @param srack_info 堆栈信息
	void finish_release_port_link_slot( NodeDirector *signal_obj_ptr, InputPort *signal_port, OutputPort *target_prot, const SrackInfo &srack_info );
	/// @brief 节点端口发生链接时，产生该信号
	/// @param signal_obj_ptr 信号对象指针
	/// @param signal_port 链接的源端口对象指针
	/// @param target_prot 链接的目标端口对象指针
	/// @param srack_info 堆栈信息
	void finish_create_port_link_slot( NodeDirector *signal_obj_ptr, InputPort *signal_port, OutputPort *target_prot, const SrackInfo &srack_info );
	/// @brief 节点依赖发生释放时候，产生该信号
	/// @param signal_obj_ptr 信号对象指针
	/// @param signal_node 依赖节点
	/// @param ref_node 被删除的依赖
	/// @param srack_info 堆栈信息
	void finish_release_ref_node_slot( NodeDirector *signal_obj_ptr, Node *signal_node, Node *ref_node, const SrackInfo &srack_info );
	/// @brief 节点依赖发生增持时候，产生该信号
	/// @param signal_obj_ptr 信号对象指针
	/// @param signal_node 依赖节点
	/// @param ref_node 被增持的依赖
	/// @param srack_info 堆栈信息
	void finish_create_ref_node_slot( NodeDirector *signal_obj_ptr, Node *signal_node, Node *ref_node, const SrackInfo &srack_info );
	/// @brief 节点错误信号
	/// @param signal_obj_ptr 信号对象指针
	...
	void error_run_node_slot( NodeDirector *signal_obj_ptr, const SrackInfo &signal_srack_info, Node *error_node, const SrackInfo &org_srack_info, NodeEnum::ErrorType error_type, const QString &error_msg );
	/// @brief 建议节点信号
	/// @param signal_obj_ptr 信号对象指针
	/// @param signal_srack_info 当前信号产生堆栈信息
	...
	void advise_run_node_slot( NodeDirector *signal_obj_ptr, const SrackInfo &signal_srack_info, Node *advise_node, const SrackInfo &org_srack_info, NodeEnum::AdviseType advise_type, const QString &advise_msg );
	/// @brief 节点完成运行信号
	/// @param signal_obj_ptr 信号对象指针
	/// @param signal_srack_info 当前信号产生堆栈信息
	/// @param finish_node 原始信号对象
	/// @param org_srack_info 原始对象堆栈信号信息
	void finish_run_node_slot( NodeDirector *signal_obj_ptr, const SrackInfo &signal_srack_info, Node *finish_node, const SrackInfo &org_srack_info );
	/// @brief 成功创建节点信号
	/// @param signal_obj_ptr 信号对象指针
	/// @param create_node 创建对象
	/// @param srack_info 堆栈信息
	void finish_create_node_slot( NodeDirector *signal_obj_ptr, Node *create_node, const SrackInfo &srack_info );
	/// @brief 节点创建失败信号
	/// @param signal_obj_ptr 信号对象指针
	...
	/// @param srack_info 堆栈信息
	void error_create_node_slot( NodeDirector *signal_obj_ptr, const QString &create_name, NodeEnum::CreateType error_type_info, const QString &error_msg, const SrackInfo &srack_info );
protected:
	MainWindow( );
	virtual bool init( );
public:
